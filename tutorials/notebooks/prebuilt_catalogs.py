...
def create_tutorial_catalog(catalog_type: PrebuiltCatalog):
    print("Cleaning up any existing tutorial catalogs")
    undeploy_all_tutorial_feature_lists()
...
def undeploy_all_feature_lists_for_catalog(catalog_name, verbose=False):
    if (verbose):
        print(f"Checking catalog: {catalog_name}")

    try:
        feature_lists = fb.Catalog.get(catalog_name).list_feature_lists()
    except:
        print(f"Warning: catalog {catalog_name} not found")
        return
    for _, feature_list in feature_lists.iterrows():

            # undeploy featurelist if it's deployed
            if feature_list["deployed"]:
                feature_list_name = feature_list["name"]
                print(f"Undeploying feature list: {catalog_name} : {feature_list_name}")

                # activate catalog and undeploy feature list
                catalog = fb.Catalog.activate(catalog_name)
                catalog.get_feature_list(feature_list_name).deploy(enable=False)

# function to undeploy all feature lists in the tutorial catalogs
def undeploy_all_tutorial_feature_lists(verbose=False):
    """
    Undeploy all deployed tutorial feature lists
    """

    # get active catalog
    current_catalog = fb.Catalog.get_active()

    # check feature lists in each catalog
    for catalog_name in fb.Catalog.list().name:
        # only check feature lists created in tutorials
        if is_tutorial_catalog(catalog_name):
            undeploy_all_feature_lists_for_catalog(catalog_name, verbose)

    # reactivate originally active catalog
    fb.Catalog.activate(current_catalog.name)

def generate_catalog_boilerplate_code():
    tables = fb.Table.list()
    print("")
    print("##################################################################")
    ...
    for i in range(tables.shape[0]):
        ...
        print(to_python_variable_name(table_name) + "_table = fb.Table.get(\"" + table_name + "\")")
    print("")
    ...
    for i in range(tables.shape[0]):
        # get the table name
        table_name = tables.iloc[i]["name"]
        print(to_python_variable_name(table_name) + "_view = " + table_name.lower() + "_table.get_view()")
...
def register_grocery_tables():
    ...
    if not catalog.list_tables().name.str.contains("GROCERYINVOICE").any():
        ...
        GroceryInvoice = fb.Table.get("GROCERYINVOICE")
    ...
    if not catalog.list_tables().name.str.contains("INVOICEITEMS").any():
        ...
        InvoiceItems = fb.Table.get("INVOICEITEMS")
    ...
    if not catalog.list_tables().name.str.contains("GROCERYPRODUCT").any():
        ...
        GroceryProduct = fb.Table.get("GROCERYPRODUCT")
...
def register_credit_card_tables():
    ...
    if not catalog.list_tables().name.str.contains("STATEDETAILS").any():
        ...
        StateDetails = fb.Table.get("STATEDETAILS")

    # check whether the data is already registered
    if not catalog.list_tables().name.str.contains("CREDITCARD").any():
        CreditCard = ds.get_table(
                    database_name="spark_catalog",
                    schema_name="CREDITCARD",
    ...
    if not catalog.list_tables().name.str.contains("CARDTRANSACTIONS").any():
        ...
        CardTransactions = fb.Table.get("CARDTRANSACTIONS")
    ...
    if not catalog.list_tables().name.str.contains("CARDFRAUDSTATUS").any():
        ...
        CardFraudStatus = fb.Table.get("CARDFRAUDSTATUS")

    # check whether the data is already registered
    if not catalog.list_tables().name.str.contains("CARDTRANSACTIONGROUPS").any():
        CardTransactionGroups = ds.get_table(
                    database_name="spark_catalog",
                    schema_name="CREDITCARD",
        ...
        CardTransactionGroups = fb.Table.get("CARDTRANSACTIONGROUPS")
...
def register_healthcare_tables():
    ...
    if not catalog.list_tables().name.str.contains(source_table_name).any():
        ...
        Patient = fb.Table.get(source_table_name)

    # check whether the data is already registered
    source_table_name = "DIAGNOSIS"
    if not catalog.list_tables().name.str.contains(source_table_name).any():
        Diagnosis = ds.get_table(
                    database_name="spark_catalog",
                    schema_name="HEALTHCARE",
                    table_name=source_table_name
                ).create_scd_table(
                    name=source_table_name,
                        ...
                        effective_timestamp_column="ValidFrom",
                        end_timestamp_column="ValidFrom",
                        record_creation_timestamp_column="record_available_at"
                )
    else:
        Diagnosis = fb.Table.get(source_table_name)

    # check whether the data is already registered
    source_table_name = "PATIENTSMOKINGSTATUS"
    if not catalog.list_tables().name.str.contains(source_table_name).any():
        PatientSmokingStatus = ds.get_table(
                    database_name="spark_catalog",
                    schema_name="HEALTHCARE",
                    table_name=source_table_name
                ).create_scd_table(
        ...
        PatientSmokingStatus = fb.Table.get(source_table_name)

    # check whether the data is already registered
    source_table_name = "ALLERGY"
    if not catalog.list_tables().name.str.contains(source_table_name).any():
        Allergy = ds.get_table(
                    database_name="spark_catalog",
                    schema_name="HEALTHCARE",
                    table_name=source_table_name
                ).create_scd_table(
        ...
        Allergy = fb.Table.get(source_table_name)
    ...
    if not catalog.list_tables().name.str.contains(source_table_name).any():
        ...
        StateDetails = fb.Table.get(source_table_name)
    ...
    if not catalog.list_tables().name.str.contains(source_table_name).any():
        ...
        Visit = fb.Table.get(source_table_name)
    ...
    if not catalog.list_tables().name.str.contains(source_table_name).any():
        ...
        Prescription = fb.Table.get(source_table_name)
    ...
    if not catalog.list_tables().name.str.contains(source_table_name).any():
        ...
        LabResult = fb.Table.get(source_table_name)
    ...
    if not catalog.list_tables().name.str.contains(source_table_name).any():
        ...
        LabObservation = fb.Table.get(source_table_name)

    # check whether the data is already registered
    source_table_name = "ICD9HIERARCHY"
    if not catalog.list_tables().name.str.contains(source_table_name).any():
        ICD9Hierarchy = ds.get_table(
                    database_name="spark_catalog",
                    schema_name="HEALTHCARE",
                    table_name=source_table_name
                ).create_dimension_table(
                    name=source_table_name,
                    dimension_id_column="ICD9Code"
                )
    else:
        ICD9Hierarchy = fb.Table.get(source_table_name)

    # check whether the data is already registered
    source_table_name = "SPECIALTYGROUP"
    if not catalog.list_tables().name.str.contains(source_table_name).any():
        SpecialtyGroup = ds.get_table(
                    database_name="spark_catalog",
                    schema_name="HEALTHCARE",
                    table_name=source_table_name
                ).create_dimension_table(
                    name=source_table_name,
                    dimension_id_column="PhysicianSpecialty"
                )
    else:
        SpecialtyGroup = fb.Table.get(source_table_name)

    # check whether the data is already registered
    source_table_name = "MEDICALPRODUCT"
    if not catalog.list_tables().name.str.contains(source_table_name).any():
        MedicalProduct = ds.get_table(
                    database_name="spark_catalog",
                    schema_name="HEALTHCARE",
                    table_name=source_table_name
                ).create_dimension_table(
        ...
        MedicalProduct = fb.Table.get(source_table_name)

    return [Patient, Diagnosis, PatientSmokingStatus, Allergy, StateDetails,
            Visit, Prescription, LabResult, LabObservation,
            ICD9Hierarchy, SpecialtyGroup, MedicalProduct]
...
def register_healthcare_entities():
    ...
    entity2 = fb.Entity.get_or_create(name="USA_state", serving_names=["STATECODE"])
    entity3 = fb.Entity.get_or_create(name="patient_smoking_status", serving_names=["PATIENTSMOKINGSTATUSGUID"])
    ...
    entity8 = fb.Entity.get_or_create(name="physician_specialty", serving_names=["PHYSICIANSPECIALTY"])
...
def tag_grocery_entities_to_columns(grocery_customer_table, grocery_invoice_table, grocery_items_table, grocery_product_table):
...
def tag_healthcare_entities_to_columns(Patient, Diagnosis, PatientSmokingStatus, Allergy, StateDetails,
            Visit, Prescription, LabResult, LabObservation,
            ICD9Hierarchy, SpecialtyGroup, MedicalProduct):
...
def create_quick_start_feature_engineering_catalog():
    ...
    if catalog_name in catalog_list['name'].values:
        ...
        fb.Catalog.activate(catalog_name)
    else:
        print('Creating new catalog')
        # creating a catalogactivates it
        catalog = fb.Catalog.create(catalog_name)
        print('Catalog created')

    print("Registering the source tables")
    [grocery_customer_table, grocery_invoice_table, grocery_items_table, grocery_product_table] = register_grocery_tables()

    print("Registering the entities")
    register_grocery_entities()

    print("Tagging the entities to columns in the data tables")
    tag_grocery_entities_to_columns(grocery_customer_table, grocery_invoice_table, grocery_items_table, grocery_product_table)
    ...
    customer_inventory_28d = grocery_items_view.groupby("GroceryCustomerGuid", category="GroceryProductGuid").aggregate_over(
        None,
        method=fb.AggFunc.COUNT,
    ...
    #generate_catalog_boilerplate_code()
...
def create_deep_dive_feature_engineering_catalog():
    ...
    inventory = grocery_items_view.groupby("GroceryCustomerGuid", category="GroceryProductGuid").aggregate_over(
        None,
        method=fb.AggFunc.COUNT,
...
def create_deep_dive_materializing_features_catalog():
    ...
    grocery_items_view.join(grocery_product_view, on="GroceryProductGuid")
    ...
    customer_inventory_4w = grocery_items_view.groupby("GroceryCustomerGuid", category="ProductGroup").aggregate_over(
        None,
        method=fb.AggFunc.COUNT,
    ...
    #customer_inventory_entropy_4w.fill_value = 0

    # get the most frequent item purchased
    customer_inventory_most_frequent_4w = customer_inventory_4w["CustomerInventory_4w"].cd.most_frequent()
    ...
    feature_list = fb.FeatureList([
        customer_inventory_entropy_4w,
        customer_inventory_most_frequent_4w,
        state_centroids
...
def create_quick_start_reusing_features_catalog():
    ...
    grocery_items_view.join(grocery_customer_view)
    ...
    state_inventory_28d = grocery_items_view.groupby("State", category="ProductGroup").aggregate_over(
        None,
        method=fb.AggFunc.COUNT,
    ...
    customer_inventory_28d = grocery_items_view.groupby("GroceryCustomerGuid", category="ProductGroup").aggregate_over(
        None,
        method=fb.AggFunc.COUNT,
        feature_names=["CustomerInventory_28d"],
        windows=['28d']
    ...
    customer_inventory_24w = grocery_items_view.groupby("GroceryCustomerGuid", category="ProductGroup").aggregate_over(
        None,
        method=fb.AggFunc.COUNT,
        feature_names=["CustomerInventory_24w"],
        windows=['24w']
    )
    ...
    invoice_unique_product_groups = grocery_items_view.groupby("GroceryInvoiceGuid", category="ProductGroup").aggregate(
        None,
        method=fb.AggFunc.COUNT,
        feature_name="InvoiceUniqueProductGroups"
    )
...
def create_playground_healthcare_catalog():
    ...
    print("Registering the source tables")
    [Patient, Diagnosis, PatientSmokingStatus, Allergy, StateDetails,
            Visit, Prescription, LabResult, LabObservation,
            ICD9Hierarchy, SpecialtyGroup, MedicalProduct] = register_healthcare_tables()

    print("Registering the entities")
    register_healthcare_entities()

    print("Tagging the entities to columns in the data tables")
    tag_healthcare_entities_to_columns(Patient, Diagnosis, PatientSmokingStatus, Allergy, StateDetails,
            Visit, Prescription, LabResult, LabObservation,
            ICD9Hierarchy, SpecialtyGroup, MedicalProduct)
...
def create_quick_start_feature_management_catalog():
    ...
    print("Setting feature list status")
...
def create_quick_start_model_training_catalog():
    ...
    # create a feature that is the inventory of the items purchased over the past 28 days, grouped by customer, subgrouped (i.e. categorized) by product group
    customer_inventory_14d = grocery_items_view.groupby("GroceryCustomerGuid", category="ProductGroup").aggregate_over(
        None,
        method=fb.AggFunc.COUNT,
        feature_names=["CustomerInventory_14d"],
        windows=['14d']
    )
    #customer_inventory_14d.save()

    customer_inventory_stability_14d28d = customer_inventory_14d["CustomerInventory_14d"].cd.cosine_similarity(customer_inventory_28d["CustomerInventory_28d"])
    customer_inventory_stability_14d28d.name = "CustomerInventoryStability_14d28d"
    customer_inventory_stability_14d28d.save()

    customer_state_similarity_28d = customer_inventory_28d["CustomerInventory_28d"].cd.cosine_similarity(state_inventory_28d["StateInventory_28d"])
    ...
    customer_avg_invoice_amount_28d = grocery_invoice_view.groupby("GroceryCustomerGuid").aggregate_over(
        value_column="Amount",
        method=fb.AggFunc.AVG,
